...
def load_transactions(filepath):
    ...
    for transaction in root.findall("transaction"):
        ...
        transactions_list.append((transaction_id, phone, store, total_transaction_price))
    
    transactions_df = pd.DataFrame(transactions_list, columns=["transaction_id", "phone", "store", "total_price"])
    transaction_items_df = pd.DataFrame(transaction_items_list, columns=["transaction_id", "item_name", "quantity", "price_per_item", "total_price"])
...
def insert_into_db(transactions, transaction_items, db_file):
    ...
    transaction_items.drop_duplicates(subset=["transaction_id", "item_name"], keep="first", inplace=True)
